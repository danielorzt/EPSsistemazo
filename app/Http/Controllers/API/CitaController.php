...
class CitaController extends Controller
{
    ...
    public function proximasCitasPorPaciente($id_paciente)
    {
        ...
        // Si el rol es paciente, asegurarse que está consultando sus propias citas
        if ($usuarioAutenticado->rol_usuario === 'paciente') {
            // Verificar que el usuario autenticado esté vinculado a un paciente
            if (!$usuarioAutenticado->paciente || $usuarioAutenticado->paciente->id != $id_paciente) {
                return response()->json([
                    'status' => false,
                    'message' => 'No autorizado para ver citas de este paciente.'
                ], 403);
            }
        }
